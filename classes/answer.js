...
import { endPointToServer, gameModeIDToName, spellIDToSpellIcon } from './swap.js';
...
exports.Answer = function (data) {
    ...
    answer.toLastLaneRequest = function () { //todo
        ...
        var server = serverToEndPoint(playerIGNAndServer[1]); //TODO: this is what every Rito API command looks like - unifize it somehow
        ...
        api.extractPlayerID(server, playerIGNAndServer, playerID => {
            ...
            api.extractPlayersLiveGameData(server, playerID, liveGameDataAPI => {
                ...
                api.extractChampionData(server, championDataAPI => {
                    ...
                    //api.extractPlayerRanksData(endPointToServer(server), ids.slice(1), ranksData => {
                    ...
                    //        return post.message(ranksData);
                        ...
                        for (var i = 0; i <= game.participants.length; i++) {
                            if (i == game.participants.length) {
                                post.embed(`${title} Live game of ${playerNickDecoded.toUpperCase()} | ${game.gameMode} ${gameModeIDToName(game.gameQueueConfigId)}`,
                            ...
                            var losses = 0;
                            var winRatio = `---`;
                            var summonerSpells = spellIDToSpellIcon(game.participants[i].spell1Id)+spellIDToSpellIcon(game.participants[i].spell2Id);
                            /*if (ranks[game.participants[i].summonerId] != undefined) {
                                rank = ranks[game.participants[i].summonerId][0].tier.substring(0, 1) + romanToArabic(ranks[game.participants[i].summonerId][0].entries[0].division);
